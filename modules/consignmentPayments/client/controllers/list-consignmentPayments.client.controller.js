(function() {
  ...
  angular
    ...
    .controller("ConsignmentPaymentsListController", ConsignmentPaymentsListController);

  ConsignmentPaymentsListController.$inject = [
    "$scope",
    "$state",
    "$window",
  ...
  function ConsignmentPaymentsListController(
    $scope,
    $state,
    $window,
    ConsignmentPaymentsService,
    ...
    ConsignmentPaymentsService.getPrimaryDetails().$promise.then(function(response) {
      ...
      vm.allConsignmentPaymentTos = response.data.bill_to;
      vm.isLoading++;
    });
      ...
      self.tableParams = new NgTableParams(
        ...
        },
        {
          ...
          getData: function($defer, params) {
            ...
            return ConsignmentPaymentsService.filteredConsignmentPayments(param).$promise.then(
              function(response) {
                vm.consignmentPayments = response.data;
                vm.totalCount = response.count;
                $defer.total(vm.totalCount);
                return vm.consignmentPayments;
              }
            );
          }
        }
    ...
    vm.gotoNewConsignmentPayment = function() {
      $state.go("consignmentPayments.create");
    };
    ...
    vm.deleteConsignmentPayment = function(id) {
      var r = confirm(
        "Are you sure you want to delete this? This action cannot be undone!"
      );
      if (r == true) {
        $http.delete("/api/consignmentPayments/" + id).then(function(response) {
          if (response.status == 200) {
            ...
            Notification.error({
              ...
              title:
                '<i class="glyphicon glyphicon-remove"></i> ConsignmentPayment deletion error!'
            });
          }
        });
      } else {
    ...
    vm.download = function(consignmentPaymentId) {
      // ConsignmentPaymentsService.downloads(consignmentPaymentId)
      // .then(function (res) {
      // var file = new Blob([res], { type: 'application/pdf' });
      var fileurl = "/api/downloads/" + consignmentPaymentId;
      window.open(fileurl, "_blank", "");
      $window.open("localhost:xxx/api/document", "_blank");
      // Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> ConsignmentPayment saved successfully!' });
      // })
      // .catch(function (res) {
      // Notification.error({ message: res.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ConsignmentPayment save error!' });
      // });
    };
