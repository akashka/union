(function () {
  ...
  angular
    .module('bookings')
    .controller('BookingsAdminCoController', BookingsAdminCoController);

  BookingsAdminCoController.$inject = ['$scope', '$state', '$window', 'BookingsService', 'Authentication', 'Notification', 'bookingResolve'];

  function BookingsAdminCoController($scope, $state, $window, booking, Authentication, Notification, bookingResolve) {
    var vm = this;
    ...
    for(var k=0; k<bookingResolve.length; k++) {
      if(bookingResolve[k]._id == $state.params.bookingId) {
            ...
            vm.booking = {
                ...
                bill_no: bookingResolve[k].bill_no + "A",
