(function() {
  ...
  angular
    ...
    .controller("OnAccountPaymentsListController", OnAccountPaymentsListController);

  OnAccountPaymentsListController.$inject = [
    "$scope",
    "$state",
    "$window",
  ...
  function OnAccountPaymentsListController(
    $scope,
    $state,
    $window,
    OnAccountPaymentsService,
    ...
    OnAccountPaymentsService.getPrimaryDetails().$promise.then(function(response) {
      ...
      vm.allOnAccountPaymentTos = response.data.bill_to;
      vm.isLoading++;
    });
      ...
      self.tableParams = new NgTableParams(
        ...
        },
        {
          ...
          getData: function($defer, params) {
            ...
            return OnAccountPaymentsService.filteredOnAccountPayments(param).$promise.then(
              function(response) {
                vm.onAccountPayments = response.data;
                vm.totalCount = response.count;
                $defer.total(vm.totalCount);
                return vm.onAccountPayments;
              }
            );
          }
        }
    ...
    vm.gotoNewOnAccountPayment = function() {
      $state.go("onAccountPayments.create");
    };
    ...
    vm.deleteOnAccountPayment = function(id) {
      var r = confirm(
        "Are you sure you want to delete this? This action cannot be undone!"
      );
      if (r == true) {
        $http.delete("/api/onAccountPayments/" + id).then(function(response) {
          if (response.status == 200) {
            ...
            Notification.error({
              ...
              title:
                '<i class="glyphicon glyphicon-remove"></i> OnAccountPayment deletion error!'
            });
          }
        });
      } else {
    ...
    vm.download = function(onAccountPaymentId) {
      // OnAccountPaymentsService.downloads(onAccountPaymentId)
      // .then(function (res) {
      // var file = new Blob([res], { type: 'application/pdf' });
      var fileurl = "/api/downloads/" + onAccountPaymentId;
      window.open(fileurl, "_blank", "");
      $window.open("localhost:xxx/api/document", "_blank");
      // Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> OnAccountPayment saved successfully!' });
      // })
      // .catch(function (res) {
      // Notification.error({ message: res.data.message, title: '<i class="glyphicon glyphicon-remove"></i> OnAccountPayment save error!' });
      // });
    };
