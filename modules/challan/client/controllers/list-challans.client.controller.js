(function() {
  ...
  angular
    .module("challans")
    .controller("ChallansListController", ChallansListController);

  ChallansListController.$inject = [
    "$scope",
    "$state",
    "$window",
    "ChallansService",
  ...
  function ChallansListController(
    $scope,
    $state,
    $window,
    ChallansService,
    ...
    ChallansService.getPrimaryDetails().$promise.then(function(response) {
      ...
      vm.allChallanTos = response.data.bill_to;
      vm.isLoading++;
    });
      ...
      self.tableParams = new NgTableParams(
        ...
        },
        {
          ...
          getData: function($defer, params) {
            ...
            return ChallansService.filteredChallans(param).$promise.then(
              function(response) {
                vm.challans = response.data;
    ...
    vm.deleteChallan = function(id) {
      var r = confirm(
        "Are you sure you want to delete this? This action cannot be undone!"
      );
      if (r == true) {
        $http.delete("/api/challans/" + id).then(function(response) {
          if (response.status == 200) {
            ...
            Notification.error({
              ...
              title:
                '<i class="glyphicon glyphicon-remove"></i> Challan deletion error!'
            });
          }
        });
      } else {
    ...
    vm.download = function(challanId) {
      // ChallansService.downloads(challanId)
      // .then(function (res) {
      // var file = new Blob([res], { type: 'application/pdf' });
      var fileurl = "/api/downloads/" + challanId;
      window.open(fileurl, "_blank", "");
      $window.open("localhost:xxx/api/document", "_blank");
      // Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> Challan saved successfully!' });
      // })
      // .catch(function (res) {
      // Notification.error({ message: res.data.message, title: '<i class="glyphicon glyphicon-remove"></i> Challan save error!' });
      // });
    };
