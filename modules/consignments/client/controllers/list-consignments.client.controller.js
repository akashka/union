(function() {
  ...
  angular
    .module("consignments")
    .controller("ConsignmentsListController", ConsignmentsListController);

  ConsignmentsListController.$inject = [
    "$scope",
    "$state",
    "$window",
    "ConsignmentsService",
  ...
  function ConsignmentsListController(
    $scope,
    $state,
    $window,
    ConsignmentsService,
    ...
    vm.deleteConsignment = function(id) {
      var r = confirm(
        "Are you sure you want to delete this? This action cannot be undone!"
      );
      if (r == true) {
        $http.delete("/api/consignments/" + id).then(function(response) {
          if (response.status == 200) {
            ...
            Notification.error({
              ...
              title:
                '<i class="glyphicon glyphicon-remove"></i> Consignment deletion error!'
            });
          }
        });
      } else {
